  ...
  render () {
    ...
    const commentsList =
      activity && comments.length > 0 ? (
        ...
        <h3>There are no Review yes, Hey!, be the first!</h3>
      )
    return activity === null ? (
      <Loader />
    ) : Object.entries(activity).length === 0 ? (
      <p>Something Horrible is happening</p>
    ) : (
      <div>
        <Header
          className="this"
          style={{
            backgroundImage: `url(http://localhost:4000${activity.thumbnail})`,
          ...
          <div className="container pt-3">
            <div className="row mb-3 container">
              <p className="h4 pr-3">
                ...
                <span className="text-secondary h6">{activity.country}</span>
              </p>
              <p className="h4">
                {' '}
                <i className="fa fa-thumb-tack text-warning" aria-hidden="true" />{' '}
                <span className="text-secondary h6">{activity.city}</span>
              </p>

              {activity.amount > 1 ? (
                <p className="ml-5 h6">
                  Strating: <span className="text-success h5">$ {activity.amount}</span>{' '}
                </p>
              ) : (
                <p className="ml-5 h6">
                  Strating: <span className="text-success h5">Free</span>{' '}
                </p>
              )}
            </div>
            <div className="row">
              <div className="col-md-8">
                ...
                <p className="p-3 description">{activity.description}</p>
              ...
              <div className="col-md-4">
                ...
                <div className="row">
                  <iframe
                    width="560"
                    height="315"
                    src="https://www.youtube.com/embed/8vxJvFqw5Uo"
                    frameborder="0"
